...
[zupt_temp T]=zero_velocity_detector(u_full);
...
%beacons=[-1 -1 0; 2 3 0; 5 7 0; 0 6 0; 5 0 0]';
beacons=[1.5 1 0]';
N_b=size(beacons,2);
radius=1.5;
[UWB, R_UWB]=generate_UWB_meas(p_opti_c,beacons,radius);
[x_h_ZUPT_UWB, q_hat_UWB]=ZUPTEKF_UWB(u_full,zupt_temp,UWB,R_UWB,beacons);
...
figure; clf;
scatter3(p_opti_full(1,:),p_opti_full(2,:),p_opti_full(3,:),10+0*traj(3,:),y_m_norm)
axis equal;
colorbar;
...
[x_h, q_hat, m, P_m]=ZUPTEKFMagSLAMLooser(u_full,zupt_temp,sigma_y,Lambda,Indices,sigma_lin,N_m,xl,xu,yl,yu,zl,zu);

[x_h_all, q_hat, m_all, P_m_all]=ZUPTEKFMagSLAMLooserUWB(u_full,zupt_temp,sigma_y,Lambda,Indices,sigma_lin,N_m,xl,xu,yl,yu,zl,zu,UWB,R_UWB,beacons);
...
caxis([0.6 1.5]);
%legend({'SLAM','ZUPT-EKF','GT','Beacon','Beacon Radius'},'Interpreter','Latex','Fontsize',13);
grid off;
axis equal;
xlim([-1.5 6]);
ylim([-2 7.5]);
...
px = beacons(1,i)-radius;
py = beacons(2,i)-radius;
rectangle('Position',[px py d d],'Curvature',[1,1],'linewidth',2,'EdgeColor','r','Linestyle','--');

%Smaller circle for center of beacon
scatter3(beacons(1,i),beacons(2,i),beacons(3,i)+10,'r','Linewidth',2);

end
plot(500:501,500:501,'r--','LineWidth',2);
fontsize=11;
plot_projection_norm_magnetic_field(m_all,P_m_all,NablaPhi3D,X,Y,Z,fontsize);
caxis([0.6 1.5]);
legend({'SLAM','ZUPT-EKF','GT','Beacon','Beacon Radius'},'Interpreter','Latex','Fontsize',fontsize,'Location','Northwest');
grid off;
axis equal;
xlim([-4 6]);
ylim([-2 7.5]);
