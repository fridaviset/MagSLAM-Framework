...
hold
y_mag_visual=u(7:9,:);
y_mag_world=zeros(size(y_mag_visual));
for i=1:size(y_mag_visual,2)
    if isnan(y_mag_visual(1,i))
       y_mag_visual(:,i)=y_mag_visual(:,i-1);
    end
    y_mag_world(:,i)=R(:,:,i)*y_mag_visual(:,i);
end
mag_norm=sqrt(sum(y_mag_visual.^2,1));

scatter3(x_h(2,:),x_h(1,:),x_h(3,:),10+0.*x_h(1,:),mag_norm);
title('Trajectory')
