function [x_h, q_hat, m, P_m]=ZUPTEKFMagSLAMLooserUWB(u,zupt,sigma_y,Lambda,Indices,sigma_lin,N_m,xl,xu,yl,yu,zl,zu,UWB,R_UWB,beacons)
...
for k=2:N
    ...
    for i=1:N_b
        if (~isnan(UWB(i,k)))
            ...
            %Find the matrix H
            diffs=(x_h(1:3,k)-beacons(:,i));
            dists=sqrt(sum(diffs.^2));
            H=(diffs./dists)';
            
            % Calculate the Kalman filter gain
            %K=((H*P(1:3,1:9))')/(H*P(1:3,1:3)*H'+R_UWB(i,i));
            ...
            % Update the filter state covariance matrix P.
            %P(1:9,1:9)=P(1:9,1:9)-K*H*P(1:3,1:9);
            P=P-K*H*P(1:3,:);
